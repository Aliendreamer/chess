...
public sealed class CookieBearerTokenResolverTests
{
    ...
    private static Task<string?> Resolve(HttpRequest request, ISessionStore store, IKeycloakOidcClient oidc, FakeClock clock) =>
        CookieBearerTokenResolver.ResolveTokenAsync(request, Cookie, store, oidc, clock, Leeway, NullLogger.Instance, CancellationToken.None);

    [Fact]
    public async Task Authorization_header_wins_and_cookie_is_ignored()
    {
        Mock<ISessionStore> store = new(MockBehavior.Strict);
        string? token = await Resolve(Request("raw", "Bearer abc"), store.Object, Mock.Of<IKeycloakOidcClient>(), new FakeClock(T0));
        Assert.Null(token);
    ...
    {
        Mock<ISessionStore> store = new(MockBehavior.Strict);
        Assert.Null(await Resolve(Request(), store.Object, Mock.Of<IKeycloakOidcClient>(), new FakeClock(T0)));
        Assert.Null(await Resolve(Request(string.Empty), store.Object, Mock.Of<IKeycloakOidcClient>(), new FakeClock(T0)));
    }
    ...
    public async Task Unknown_or_revoked_session_is_anonymous()
    {
        ...
        store.Setup(s => s.GetValidAsync("raw", It.IsAny<CancellationToken>())).ReturnsAsync((UserSession?)null);
        Assert.Null(await Resolve(Request("raw"), store.Object, Mock.Of<IKeycloakOidcClient>(), new FakeClock(T0)));
    }
    ...
    {
        Mock<ISessionStore> store = new();
        store.Setup(s => s.GetValidAsync("raw", It.IsAny<CancellationToken>())).ReturnsAsync(Session(T0.AddMinutes(5)));
        Mock<IKeycloakOidcClient> oidc = new(MockBehavior.Strict);

        Assert.Equal("access-1", await Resolve(Request("raw"), store.Object, oidc.Object, new FakeClock(T0)));
    ...
    {
        Mock<ISessionStore> store = new();
        store.Setup(s => s.GetValidAsync("raw", It.IsAny<CancellationToken>())).ReturnsAsync(Session(T0.AddSeconds(10), "rt", T0.AddHours(1)));
        ...
        string? token = await Resolve(Request("raw"), store.Object, oidc.Object, new FakeClock(T0));
    ...
    {
        Mock<ISessionStore> store = new();
        store.Setup(s => s.GetValidAsync("raw", It.IsAny<CancellationToken>())).ReturnsAsync(Session(T0.AddMinutes(-1)));
        Mock<IKeycloakOidcClient> oidc = new();
        oidc.Setup(o => o.RefreshAsync("rt", It.IsAny<CancellationToken>()))
            .ReturnsAsync(new TokenResponse("access-2", null, 300, 0, null));

        Assert.Equal("access-2", await Resolve(Request("raw"), store.Object, oidc.Object, new FakeClock(T0)));
    ...
    {
        Mock<ISessionStore> store = new();
        store.Setup(s => s.GetValidAsync("raw", It.IsAny<CancellationToken>())).ReturnsAsync(Session(T0.AddMinutes(-1), refresh: null));
        Mock<IKeycloakOidcClient> oidc = new(MockBehavior.Strict);

        Assert.Null(await Resolve(Request("raw"), store.Object, oidc.Object, new FakeClock(T0)));
    }
    ...
    {
        Mock<ISessionStore> store = new();
        store.Setup(s => s.GetValidAsync("raw", It.IsAny<CancellationToken>())).ReturnsAsync(Session(T0.AddMinutes(-1), "rt", T0.AddMinutes(-1)));
        Mock<IKeycloakOidcClient> oidc = new(MockBehavior.Strict);
