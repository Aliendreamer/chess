# api-token-validation Specification

## Purpose

Which access tokens the backend accepts. The issuer must be the realm, and the audience must include
`chess_api`, in every deployed environment. Both calling clients carry an audience mapper: `chess_api` (user
login) and `chess_bff` (relay service account). Realm roles gate the live hub, which admits only `Relay`.
Established by change `shared-realtime-relay` (2026-09-25).
