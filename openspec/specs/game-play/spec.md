...
What a live game guarantees. It covers who may act and when, moves validated by the rules library and recorded
self-describing (UCI, SAN, FEN, clocks), server clocks with increment, flag fall without a message, failover
forgiveness, every way a game ends, aborting unplayed games, and every event reaching Kafka and the live relay.
Established by change `game-core` (2026-09-26).
