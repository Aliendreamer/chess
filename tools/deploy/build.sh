...
#
# Registry credentials (push mode only) come from the environment: REGISTRY, REGISTRY_USER,
# REGISTRY_PASS. Source them from your own credential store before calling this — deliberately no
...
# aliendreamer/ prefix; REGISTRY is the bare registry host (docker.io), so
# FULL_IMAGE = docker.io/aliendreamer/<name>.
case "$APP" in
  proxy)
...
: "${REGISTRY:?Missing REGISTRY — export it or supply it from your credential store}"
: "${REGISTRY_USER:?Missing REGISTRY_USER — export it or supply it from your credential store}"
: "${REGISTRY_PASS:?Missing REGISTRY_PASS — export it or supply it from your credential store}"
...
echo "$REGISTRY_PASS" | $CONTAINER_RUNTIME login "$REGISTRY" -u "$REGISTRY_USER" --password-stdin >&2
